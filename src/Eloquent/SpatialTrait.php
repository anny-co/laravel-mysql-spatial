...
trait SpatialTrait
{
    ...
    protected function performInsert(EloquentBuilder $query, array $options = [])
    {
        foreach ($this->attributes as $key => $value) {
            if ($value instanceof GeometryInterface) {
                ...
                $this->attributes[$key] = new SpatialExpression($value, $this->getConnection());
            }
        }
    ...
    public function scopeDistance($query, $geometryColumn, $geometry, $distance)
    {
        ...
        $expression = ExpressionGenerator::getExpression($query->getConnection());
        $query->whereRaw("st_distance(`$geometryColumn`, {$expression}) <= ?", [
            $geometry->toWkt(),
            $geometry->getSrid(),
            $distance,
        ]);
    ...
    public function scopeDistanceExcludingSelf($query, $geometryColumn, $geometry, $distance)
    {
        ...
        $expression = ExpressionGenerator::getExpression($query->getConnection());
        $query->whereRaw("st_distance(`$geometryColumn`, {$expression}) != 0", [
            $geometry->toWkt(),
            $geometry->getSrid(),
        ]);
    ...
    public function scopeDistanceValue($query, $geometryColumn, $geometry)
    {
        ...
        $expression = ExpressionGenerator::getExpression($query->getConnection());
        $query->selectRaw("st_distance(`$geometryColumn`, {$expression}) as distance", [
            $geometry->toWkt(),
            $geometry->getSrid(),
        ]);
    }
    ...
    public function scopeDistanceSphere($query, $geometryColumn, $geometry, $distance)
    {
        ...
        $expression = ExpressionGenerator::getExpression($query->getConnection());
        $query->whereRaw("st_distance_sphere(`$geometryColumn`, {$expression}) <= ?", [
            $geometry->toWkt(),
            $geometry->getSrid(),
            $distance,
        ]);
    ...
    public function scopeDistanceSphereExcludingSelf($query, $geometryColumn, $geometry, $distance)
    {
        ...
        $expression = ExpressionGenerator::getExpression($query->getConnection());
        $query->whereRaw("st_distance_sphere($geometryColumn, {$expression}) != 0", [
            $geometry->toWkt(),
            $geometry->getSrid(),
        ]);
    ...
    public function scopeDistanceSphereValue($query, $geometryColumn, $geometry)
    {
        ...
        $expression = ExpressionGenerator::getExpression($query->getConnection());
        $query->selectRaw("st_distance_sphere(`$geometryColumn`, {$expression}) as distance", [
            $geometry->toWkt(),
            $geometry->getSrid(),
        ]);
    }
    ...
    public function scopeComparison($query, $geometryColumn, $geometry, $relationship)
    {
        ...
        $expression = ExpressionGenerator::getExpression($query->getConnection());
        $query->whereRaw("st_{$relationship}(`$geometryColumn`, {$expression})", [
            $geometry->toWkt(),
            $geometry->getSrid(),
        ]);
    ...
    public function scopeOrderBySpatial($query, $geometryColumn, $geometry, $orderFunction, $direction = 'asc')
    {
        ...
        $expression = ExpressionGenerator::getExpression($query->getConnection());
        $query->orderByRaw("st_{$orderFunction}(`$geometryColumn`, {$expression}) {$direction}", [
            $geometry->toWkt(),
            $geometry->getSrid(),
        ]);
    ...
    protected function getST_GeomFromTextExpressionFromQuery($query){
